...
| 9 | **Gom trùng theo lượt xe** | Một lượt xe bị tách/mất-track ra nhiều mảnh gây thừa; nhưng cùng biển cách xa (ra rồi vào lại, hoặc 2 xe đọc trùng chuỗi) là **2 lượt khác nhau** không được nuốt vào 1 dòng | `consolidate_vehicle_events` (`cross_plate_merge_gap_ms=90s`), dedup lúc đọc (`_merge_persisted_motion_candidates`) | Gộp mảnh cùng biển **chỉ khi biển bị mất-detection ≤90s** (một lượt liên tục, kể cả dừng lâu/bị che), cửa sổ span đúng cả lượt; cách xa hơn → giữ **2 event riêng** + gắn cờ **`REPEATED_PLATE`** (vào "Cần xem lại", không tự duyệt) để người kiểm tra thật/nhầm; không mất xe, không tạo "bỏ sót" giả; suppress mảnh biển trùng/che phủ ngắn, card false-positive, false-positive không phải biển (vạch/đèn/logo), mảnh unreadable cạnh biển rõ; frame tốt hơn thay crop nhưng không tạo dòng mới trong cùng lượt |
